...
function computeFilterId(csp, mask, filter, hostname, domains, denyallow, redirect) {
    let hash = (utils_1.HASH_SEED * utils_1.HASH_INTERNAL_MULT) ^ mask;
    if (csp !== undefined) {
        for (let i = 0; i < csp.length; i += 1) {
            hash = (hash * utils_1.HASH_INTERNAL_MULT) ^ csp.charCodeAt(i);
        }
    }
    ...
    if (filter !== undefined) {
        for (let i = 0; i < filter.length; i += 1) {
            hash = (hash * utils_1.HASH_INTERNAL_MULT) ^ filter.charCodeAt(i);
        }
    }
    if (hostname !== undefined) {
        for (let i = 0; i < hostname.length; i += 1) {
            hash = (hash * utils_1.HASH_INTERNAL_MULT) ^ hostname.charCodeAt(i);
        }
    }
    if (redirect !== undefined) {
        for (let i = 0; i < redirect.length; i += 1) {
            hash = (hash * utils_1.HASH_INTERNAL_MULT) ^ redirect.charCodeAt(i);
        }
    }
    return hash >>> 0;
}
